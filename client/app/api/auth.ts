...
export async function postToken(credentials: LoginCredentials) {
	const body = new URLSearchParams();

	body.set("username", credentials.username);
	body.set("password", credentials.password);

	return await $fetch<TokenResponse>(`${getServerApi()}/auth/token`, {
		method: "POST",
		headers: {
...
export async function postTokenRefresh(refreshToken: string) {
	return await useFetch<TokenRefreshResponse>(
		`${getServerApi()}/auth/token/refresh`,
		{
			method: "POST",
			query: { refreshToken },
		}
...
export async function postSignup(user: SignUpDetails) {
	return await useFetch(`${getServerApi()}/auth/signup`, {
		method: "POST",
